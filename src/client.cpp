...
inline scrcpy::client::client(std::string addr, const std::uint16_t port): addr(std::move(addr)), port(port) {
}

inline scrcpy::client::~client() {
    server_c.terminate();
}
...
inline std::tuple<std::uint64_t, std::uint64_t> scrcpy::client::get_w_size() {
    return {width, height};
}
...
inline std::optional<std::string> scrcpy::client::forward_list_contains_tcp_port(const std::filesystem::path &adb_bin,
...
inline auto scrcpy::client::deploy(const std::filesystem::path &adb_bin, const std::filesystem::path &scrcpy_jar_bin,
